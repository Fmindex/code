  ...
  for (int lap = 1; lap <= t; lap++)
  {
    ...
    for (int i = 1; i <= n; i++) {
      ...
      if (i > 1) {
        ...
        sum3 = a[i][2] + a[i - 1][2];
        for (int j = 1; j <= min(k, i); j++) {
          if (i > 2) dp[i][j] = max(dp[i][j], max(dp[i - 1][j], dp[i - 2][j]));
          if (j >= 3) dp[i][j] = max(dp[i][j], sum1 + sum2 + sum3 + dp[i - 2][j - 3]);
          if (j >= 2) dp[i][j] = max(dp[i][j], sum1 + sum3 + dp[i - 2][j - 2]);
          if (j >= 2) dp[i][j] = max(dp[i][j], sum1 + sum2 + dp[i - 2][j - 2]);
          if (j >= 2) dp[i][j] = max(dp[i][j], sum2 + sum3 + dp[i - 2][j - 2]);
